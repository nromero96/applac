$(document).ready(function() {
    ...
    $('#formservicesupplier').submit(function(e) {
        ...
        $('#listroutes .card').each(function() {
            ...
            var originCountry = $(this).find('select[name="origin_country_id[]"]').val();
            var originState = $(this).find('select[name="origin_state_id[]"]').val();
            var originCity = $(this).find('input[name="origin_city[]"]').val();
            var destinationCountry = $(this).find('select[name="destination_country_id[]"]').val();
            var destinationState = $(this).find('select[name="destination_state_id[]"]').val();
            var destinationCity = $(this).find('input[name="destination_city[]"]').val();
            var crossing = $(this).find('select[name="crossing[]"]').val();
            var returnRoute = $(this).find('input[name="return_route[]"]').is(':checked');
    ...
    $('#add_route').click(function() {
        ...
        //get html
        var html = '<div class="col-md-6 routeitem mb-2">'+
                                                            '<div class="card p-1">'+
                                                            ...
                                                            '<input class="form-check-input" name="return_route[]" type="checkbox" id="rtreturn'+n+'">'+
                                                            '<label class="form-check-label" for="rtreturn'+n+'"> Return</label>'+
                                                                              '</div>'+
                                                                              '</div>'+
                                                                              '</div>'+
                                                                              '</div>'+
        ...
        $.ajax({
            ...
            success: function(data) {
                    ...
                    //add in this route
                    $('.routeitem:last-child select[name="origin_country_id[]"]').html(html);
                    $('.routeitem:last-child select[name="destination_country_id[]"]').html(html);
    ...
    $('#add_location').click(function() {
        var n = $('.routeitem').length + 1;
        //get html
        var html = '<div class="col-md-4 routeitem mb-2">'+
                                                        '<div class="card p-1">'+
                                                            '<button type="button" class="btn btn-danger p-0 btn-sm remove_route">X</button>'+
                                                                '<div class="card-body p-2">'+
                                                                    '<div class="row">'+
    ...
    $(document).on('click', '.remove_route', function() {
        ...
        $('.routeitem').each(function(index) {
            var n = index + 1;
            $(this).find('input[name="return_route[]"]').attr('id', 'rtreturn' + n);
            $(this).find('label.form-check-label').attr('for', 'rtreturn' + n);
        }
        );
                ...
                if (data.length > 0) {
                    ...
                    for (var i = 0; i < data.length; i++) {
                        ...
                        for (var j = 0; j < services.length; j++) {
                            ...
                            serviceBadges.push('<span class="badge badge-light-danger mb-1 me-1">' + service.name + '</span>');
                        }
                        ...
                        for (var k = 0; k < routes.length; k++) {
                            ...
                            var origin_state_name = route.origin_state_name ? ', ' + route.origin_state_name : '';
                            var origin_city_name = route.origin_city ? ', ' + route.origin_city : '';
                        
                            var destination_country_name = route.destination_country_name;
                            var destination_state_name = route.destination_state_name ? ', ' + route.destination_state_name : '';
                            var destination_city_name = route.destination_city ? ', ' + route.destination_city : '';
                            var crossing = route.crossing_name ? 'Crossing: ' + route.crossing_name : '';
                        
                            var icon = route.return_route == 1 ? '↔' : '→';
                        
                            var routeItem = origin_country_name + origin_state_name + origin_city_name;
                        
                            if (destination_country_name || destination_state_name || destination_city_name) {
                                routeItem += ' <b>' + icon + '</b> ';
                            ...
                            routeItems.push('<label class="list-group-item">' + routeItem + '</label>');
                        }
                        ...
                        htmlArray.push(
                            ...
                            '<div class="badge badge-dot" style="background-color:' + item.category_color + '"></div>' +
                            '</div>' +
                            '<div class="media-body">' +
                            '<h4 class="media-heading mb-0">' +
                            '<span class="media-title">' + item.category_name + '</span>' +
                    ...
                    dvelemt.html('<div class="col-md-12"><div class="alert alert-danger">No services</div></div>');
                }
            }
        });
    }
    ...
    $(document).on('click', '.editservice', function() {
        ...
        $('#listroutes').html('<div class="text-center"><div class="spinner-grow text-warning align-self-center"></div></div>');
        ...
        $.ajax({
          ...
          success: function(response) {
            $('input[name="service_supplier_id"]').val(response.servicesupplier.id);
            $('#servicecategory_id').val(response.servicesupplier.servicecategory_id);
            var services_list = JSON.parse(response.servicesupplier.services_list);
            var services_list_name = [];
            for (var i = 0; i < services_list.length; i++) {
              services_list_name.push(services_list[i].name);
            }
      
            var serviceCategoryId = response.servicesupplier.servicecategory_id;
            fetchWhitelistData(serviceCategoryId).then(function(data) {
              ...
              inputServicesList[0].addEventListener('add', function(event) {
            ...
            $.ajax({
              ...
              success: function(data) {
                ...
                for (var i = 0; i < routes.length; i++) {
                  ...
                  $.ajax({
                    url: baseurl + '/getstates/' + selectedDestination,
                    type: 'GET',
                    async: false,
                    success: function(data) {
                      var html = '<option value="">State...</option>';
                      $.each(data, function(index, element) {
                        if (element.id == route.destination_state) {
                          html += '<option value="' + element.id + '" selected>' + element.name + '</option>';
                        } else {
                          html += '<option value="' + element.id + '">' + element.name + '</option>';
                        }
                    ...
                    $.ajax({
                      ...
                      success: function(data) {
                        if (data.length > 0) {
                          ...
                          var html = '<option value="">Crossing...</option>';
                          data.forEach(element => {
                            if (element.id == route.crossing) {
                              html += '<option value="' + element.id + '" selected>' + element.crossing_name + '</option>';
                            } else {
                              html += '<option value="' + element.id + '">' + element.crossing_name + '</option>';
                            }
                          });
                  ...
                  var checked = route.return_route == '1' ? 'checked' : '';
                            ...
                            <div class="col-md-6">
                              ...
                              <div class="form-group mt-1 mb-1">
                                <select class="form-control origin_country" name="origin_country_id[]" required="">
                                  ${html.replace('<option value="' + selectedOrigin + '">', '<option value="' + selectedOrigin + '" selected>')}
                                </select>
                              </div>
                              <div class="form-group mt-1 mb-1">
                              ...
                              <div class="form-group mt-1 mb-1">
                                <input type="text" name="origin_city[]" class="form-control form-control-sm" value="${route.origin_city}" placeholder="City">
                              </div>
                            </div>
                            <div class="col-md-6">
                              <label class="form-label fw-bold">Destination:</label>
                              <div class="form-group mt-1 mb-1">
                                <select class="form-control destination_country" name="destination_country_id[]" required="">
                                  ${html.replace('<option value="' + selectedDestination + '">', '<option value="' + selectedDestination + '" selected>')}
                                </select>
                              </div>
                              <div class="form-group mt-1 mb-1">
                              ...
                              <div class="form-group mt-1 mb-1">
                                <input type="text" name="destination_city[]" class="form-control form-control-sm" value="${route.destination_city}" placeholder="City">
                              </div>
                            </div>
                          </div>
...
document.getElementById('print_supplier').addEventListener('click', function() {
